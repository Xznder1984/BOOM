/*
 * BOOM - Level data and loading
 * Built-in levels, no WAD files needed
 */
#include "boom.h"
...
static void level_add_item(Level *lvl, float x, float y, int type) {
    ...
    switch (type) {
        case ITEM_HEALTH:
            it->amount = 10;
            snprintf(it->name, sizeof(it->name), "Health Bonus"); break;
        case ITEM_MEDKIT:
            it->amount = 25;
            snprintf(it->name, sizeof(it->name), "Medikit"); break;
        case ITEM_ARMOR:
            it->amount = 100;
            snprintf(it->name, sizeof(it->name), "Security Armor"); break;
        case ITEM_COMBAT_ARMOR:
            it->amount = 200;
            snprintf(it->name, sizeof(it->name), "Combat Armor"); break;
        case ITEM_CLIP:
            it->ammo_type = 0; it->amount = 10;
            snprintf(it->name, sizeof(it->name), "Clip"); break;
        case ITEM_SHELLS:
            it->ammo_type = 1; it->amount = 4;
            snprintf(it->name, sizeof(it->name), "Shells"); break;
        case ITEM_ROCKETS:
            it->ammo_type = 2; it->amount = 2;
            snprintf(it->name, sizeof(it->name), "Rockets"); break;
        case ITEM_CELLS:
            it->ammo_type = 3; it->amount = 20;
            snprintf(it->name, sizeof(it->name), "Cell Pack"); break;
        case ITEM_SHOTGUN:
            it->weapon_type = WPN_SHOTGUN;
            it->color_r = 0; it->color_g = 200; it->color_b = 200;
            snprintf(it->name, sizeof(it->name), "Shotgun"); break;
        case ITEM_CHAINGUN:
            it->weapon_type = WPN_CHAINGUN;
            it->color_r = 0; it->color_g = 200; it->color_b = 200;
            snprintf(it->name, sizeof(it->name), "Chaingun"); break;
        case ITEM_ROCKET_LAUNCHER:
            it->weapon_type = WPN_ROCKET;
            it->color_r = 0; it->color_g = 200; it->color_b = 200;
            snprintf(it->name, sizeof(it->name), "Rocket Launcher"); break;
        case ITEM_PLASMA:
            it->weapon_type = WPN_PLASMA;
            it->color_r = 0; it->color_g = 200; it->color_b = 200;
            snprintf(it->name, sizeof(it->name), "Plasma Rifle"); break;
        case ITEM_BFG:
            it->weapon_type = WPN_BFG;
            it->color_r = 0; it->color_g = 255; it->color_b = 0;
            snprintf(it->name, sizeof(it->name), "BFG 9000"); break;
        case ITEM_KEY_RED:
            it->key_color = 0;
            it->color_r = 255; it->color_g = 0; it->color_b = 0;
            snprintf(it->name, sizeof(it->name), "Red Key"); break;
        case ITEM_KEY_BLUE:
            it->key_color = 1;
            it->color_r = 0; it->color_g = 0; it->color_b = 255;
            snprintf(it->name, sizeof(it->name), "Blue Key"); break;
        case ITEM_KEY_YELLOW:
            it->key_color = 2;
            it->color_r = 255; it->color_g = 255; it->color_b = 0;
            snprintf(it->name, sizeof(it->name), "Yellow Key"); break;
    }
    lvl->num_items++;
}
